...
We developed it with Kutalmış İnce to mobilize the curtains in our bedrooms so that they can be programmed to open up at (say) 7AM and closes down at (say) 6PM. We also wanted them to be manually controllable via simple push buttons or an online interface. And this became a long journey in which we first developed a master/slave configuration where a Raspberry Pi acted as the main hub, interfacing the user online and directing some Atmel based controllers. We would put Raspberry Pi in close proximity of the router and it will connect with the controllers using nRF24L01+ ICs. First problem was to overcome the range problem which we solved by implementing an internal mesh network of Atmel+nRF24L01 nodes. In the end, we get our system working, however it became a bit expensive, and we thought that we could construct the same system using ESP8266. The range problem was still there, so we set a mesh network again consisting of the same ESP8266 development boards. This was cheaper and working, but we were using nodeMCU devboards (which use ESP8266-12 modules with lots of GPIO pins) and this could still get cheaper by using ESP8266-01 modules. This wasn't easy as said, since ESP8266-01 is a very limited module. Rest of this document explains the whole procedure to get such a system up and running.  
